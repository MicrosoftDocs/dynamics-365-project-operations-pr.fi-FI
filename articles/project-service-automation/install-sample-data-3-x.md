...
4. Jos et käytä Spencer Low'n käyttäjätunnusta **spencerl**, sinun on päivitettävä toinen tiedosto. Avaa **DemoDataPreImportConfig.xml**-tiedosto ja etsi **userstocreateandconfigure**-tunniste. Päivitä **\<sisäänkirjauksen\>** tunnisteeksi käyttäjän Spencer Low käyttäjätunnus. Lisätietoja on [teknisissä huomautuksissa](#technical-notes).
...
- **\<EntitiesToEnableScheduling\>** määrittää, mitkä Microsoft Dynamics -entiteetit otetaan käyttöön aikataulutuksessa (aka Universal Resource Scheduling).
